...
class Model(object):
    ...
    def preprocess(self, inputs, resizeWidth, resizeHeight):
        """preprocessing.
        ...
        preprocessed_inputs = tf.image.resize_images(inputs, [resizeWidth, resizeHeight])
        # preprocessed_inputs = tf.cast(preprocessed_inputs, tf.float32)
        # preprocessed_inputs = CustomImagePreprocess.preprocess_image(inputs,
        #                                        height=resizeHeight,
        #                                        width=resizeWidth,
        #                                        is_training=True,
        #                                        fast_mode=False,
    ...
    def predict(self, preprocessed_inputs):
        ...
        Returns:
            prediction_dict: A dictionary holding prediction tensors to be
                passed to the Loss or Postprocess functions.
        """
        # with slim.arg_scope(nets.resnet_v1.resnet_arg_scope()): #resnet50W
        #     net, endpoints = nets.resnet_v1.resnet_v1_101(
        #         preprocessed_inputs, num_classes=None,
        #         is_training=self._is_training)
        with slim.arg_scope(nets.inception.inception_v3_arg_scope()): #inceptionV3W
            logits, endpoints = nets.inception.inception_v3(
                preprocessed_inputs, num_classes=self._num_classes,
                is_training=self._is_training)
        # with slim.arg_scope(inception_v4.inception_v4_arg_scope()): #inceptionV4W
