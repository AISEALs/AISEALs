...
from common_tools.categorical_vocabulary import CategoricalVocabulary  # pylint: disable=g-bad-import-order
...
if __name__ == '__main__':
    ...
    data_dir = ""
    from data_prepare.vocabulary_processor import load_data_and_labels
    x_text = load_data_and_labels(data_dir, max_document_length)
    vocab.fit(x_text)

    vocab.save('vocab.pickle')

    line = "大妈的皇副卡二，他们各个都是新世界里叱诧风云的人物，然而这次小编想着重讲的两个皇"
    vocab_dir = "/Users/jiananliu/work/python/AISEALs/text_classification/files/vocab.pickle"
    vocab = VocabularyProcessor.restore(vocab_dir)
    # 文本转为词ID序列，未知或填充用的词ID为0
    id_documents = list(vocab.transform([line]))
    print(id_documents[0])
