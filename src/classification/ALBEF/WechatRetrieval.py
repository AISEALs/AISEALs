...
def main(args, config):
    ...
    train_dataset, val_dataset, test_dataset = create_dataset('wechat', config)
    ...
    model = ALBEF(args=args, config=config, text_encoder=args.text_encoder, tokenizer=tokenizer)
    ...
    if args.distributed:
        # 重点2：用DistributedDataParallel封装模型,指定每个进程使用的GPU，不设置device_ids时默认为本节点所有GPU，会出问题“mismatch
    ...
    for epoch in range(0, max_epoch):
        ...
        if args.distributed:
            dist.barrier()
            # barrier类似一个路障，进程会被拦住，直到所有进程都集合齐了才放行。适合下面几种场景：
            #   - 只一个进程下载，其他进程可以使用下载好的文件。在下载后使用barrier
            #   - 只一个进程预处理数据，其他进程使用预处理且cache好的数据。在预处理后使用barrier
            #   - 只有一个进程保存模型，模型保存需要用到其他进程的计算结果。在保存模型之前使用barrier
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--text_encoder', default='data/chinese-macbert-base')
    parser.add_argument('--text_encoder', default='data/bert-base-uncased')
