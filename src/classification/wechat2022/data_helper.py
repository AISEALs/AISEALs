...
class MultiModalDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(args.bert_dir, user_fast=True)
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # ocr_list = self.anns[idx]['ocr']
        # if len(ocr_list) > 0:
        #     i = np.random.randint(0, len(ocr_list))
        #     ocr = ocr_list[i]['text']
        # else:
        #     ocr = ""
        ...
        # )
        ocr_list = self.anns[idx]['ocr']
        text = self.anns[idx]['title'] + self.anns[idx]['asr'] + ''.join([ocr['text'] for ocr in ocr_list])
        if len(text) > self.bert_seq_length - 3:
            # text = text[0:128] + context[-128:]
            # 取top256, title和asr信息为全局信息，比较重要
            text = text[0:self.bert_seq_length - 3]
        #doc_input, doc_mask = self.tokenize_text(context)
        doc_encode = self.tokenizer.encode_plus('', text,
                                add_special_tokens=True,
                                max_length=self.bert_seq_length,
                                padding='max_length',
                                truncation = 'longest_first')
        # 构造为[CLS][SEP] 文本 [SEP], 方便后续跟video拼接构建成[CLS] 图像 [SEP] 文本 [SEP]
        doc_input = torch.LongTensor(doc_encode['input_ids'])
        doc_mask = torch.LongTensor(doc_encode['attention_mask'])
        data = dict(
            frame_input=frame_input,
            frame_mask=frame_mask,
            doc_input=doc_input,
