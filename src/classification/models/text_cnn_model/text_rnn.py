...
class TextRNN(object):
    ...
    def __init__(self, vocab_size, embedding_size, sequence_length, rnn_size, num_layers,
        attention_size, num_classes, learning_rate, grad_clip,dropout_keep_prob,batch_size,features=None,labels=None):
        """
        ...
        self.seq_len = tf.constant([512 for x in range(batch_size)],name='seq_len') #tf.placeholder(tf.int32, shape=[None], name='seq_len')
        self.keep_prob = dropout_keep_prob #tf.placeholder(tf.float32, name='keep_prob')
        ...
        with tf.device('/gpu:0'), tf.name_scope('embedding'):
            ...
            #self.embedded_chars_expanded = tf.expand_dims(self.embedding_inputs, -1)
        ...
        with tf.name_scope('bi_rnn'):
            ...
            rnn_output, _ = tf.nn.bidirectional_dynamic_rnn(fw_rnn_cell, bw_rnn_cell, inputs=embedding_inputs,dtype=tf.float32)
