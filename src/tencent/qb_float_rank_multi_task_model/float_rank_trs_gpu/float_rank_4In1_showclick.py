...
import utils
from layers import MMoEModel, DenoiseLayer, HadamardLayer, AutoIntModel, AutoGroupLayer, ShowClickLayer
...
import os
from config import slots_showclick as slots
...
use_a10 = int(get_config().get_option("userdefine.config.use_a10", default_value="0"))
...
print('use_a10: {}'.format(use_a10))

if use_a10:
    from config.config_a10 import get_gpu_config, get_common_args
else:
...
class DNNModel(tf.keras.Model):
    def __init__(self, **kwargs):
        ...
        self.show_click_layer = ShowClickLayer(slots=slots.show_click_slots,
                                               filter_slots=slots.show_click_filter_slots,
                                               replace_slot=slots.replaced_slot,
                                               labels=slots.show_click_labels,
                                               dim=slots.show_click_emb_dim,
                                               task_names=task_names,
                                               norm_type=args.showclick_norm_type,
                                               dropout_rate=args.showclick_drop_rate,
                                               use_share=args.showclick_use_share,
                                               debug_mode=args.debug_mode,
                                               decay_weight=args.showclick_decay_weight,
                                               )
        self.denoise_net = {}
    ...
    def call(self, inputs, training):
        ...
        if args.showclick_check:
            self.show_click_layer.check(inputs)
            show_click_check_emb = [inputs[f'show_click_api_{slot}'] for slot in slots.show_click_slots]
            ...
            embedding_merge_layer = tf.concat([attention_output] + sparse_features +
                                              hadamard_deep_outs + auto_group_outputs +
        ...
        ratio_out = tf.add_n([wide_outs[0], hadamard_wide_outs[0], show_click_wide_outs[0], deep_outs[0]])
        ratio_pred = tf.clip_by_value(tf.sigmoid(ratio_out) * biz_weight, clip_value_min=0.0, clip_value_max=1.0)
        skip_out = tf.add_n([wide_outs[1], hadamard_wide_outs[1], show_click_wide_outs[1], deep_outs[1]])
        skip_pred = tf.clip_by_value(tf.sigmoid(skip_out) / biz_weight, clip_value_min=0.0, clip_value_max=1.0)
        finish_out = tf.add_n([wide_outs[2], hadamard_wide_outs[2], show_click_wide_outs[2], deep_outs[2]])
        finish_pred = tf.clip_by_value(tf.sigmoid(finish_out) * biz_weight, clip_value_min=0.0, clip_value_max=1.0)
        time_out = tf.add_n([wide_outs[3], hadamard_wide_outs[3], show_click_wide_outs[3], deep_outs[3]])
...
class QBMiniFloatModel(NumerousModel):
    ...
    def build_inputs(self):
        ...
        sum_combiner = numerous.layers.Combiner.SUM
        ...
        show_click_optimizer = numerous.optimizers.GlobalSum(summary_decay_rate=args.showclick_decay_rate)
        inputs = Inputs()
        ...
        for slot_id in filter(lambda slot: slot not in slots.show_click_filter_slots, slots.show_click_slots):
            if args.showclick_check:
                inputs.add_sparse_feature(
            ...
            if not args.showclick_use_share:
                inputs.add_sparse_feature(
                    feature_name=f'show_click_handcraft_{slot_id}',
                    slot_id=slot_id,
                    embedding_dim=slots.show_click_emb_dim,
                    optimizer=show_click_optimizer,
                    initializer=show_click_initializer,
                    combiner=sum_combiner
                )

        if args.showclick_use_share:
            merge_output = False if args.showclick_check else True
            show_click_feature_names = [f"show_click_handcraft_{slot_id}" for slot_id in slots.show_click_slots]
            # # 手动定义show_click table，1个show，n个目标，合计n+1维。
            inputs.add_share_embedding_features(
                feature_names=show_click_feature_names,
                slot_ids=slots.show_click_slots,
                embedding_dim=slots.show_click_emb_dim,
    ...
    def compute_loss(self, labels, model_outputs, sample_weights, training):
        ...
        if args.showclick_use_share:
            show_click_grads = tf.tile(tf.expand_dims(show_click_grads, axis=1), [1, len(slots.show_click_slots), 1])
