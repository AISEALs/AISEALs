...
from layers import WatchTimeGainUpdater
...
duration_boundaries = [7, 9, 11, 15, 20, 25, 30, 40, 50, 60, 80, 100, 200, 300, 500]
...
class Runner(NumerousModel):
    def __init__(self, name):
        ...
        self.wtg_updater = WatchTimeGainUpdater(bin_boundaries=duration_boundaries, name="ratio_wtg")
    ...
    def compute_loss(self, labels, model_outputs, training):
        ...
        ratio_label = self.wtg_updater([ratio_label, video_time], name='ratio_label')
        ...
        time_loss_ce = tf.compat.v1.losses.sigmoid_cross_entropy(time_label, time_logits, sample_weights_finish,
                                                                 reduction=tf.compat.v1.losses.Reduction.NONE)
        ratio_loss_mse = tf.compat.v1.losses.mean_squared_error(ratio_label, ratio_logits, sample_weights_finish,
                                                                reduction=tf.compat.v1.losses.Reduction.NONE)
        ...
        final_loss = skip_loss_ce + finish_loss_ce + ratio_loss_mse + time_loss_ce + \
                     denoise_loss_finish + denoise_loss_skip + \
                     amm_loss * amm_weight
