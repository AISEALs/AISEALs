...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_attention_probs=False):
        ...
        if output_attention_probs:
            ...
            return context_layer # [bs, seqlen, hid_size] 得到输出
...
class BertLayer(nn.Module):
    def __init__(self, config):
        ...
        self.attention = BertAttention(config)  # self attention网络
        self.intermediate = BertIntermediate(config)    # feed forward网络
        self.output = BertOutput(config) # output网络，即Add&Norm
...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # to the first token.
        # 使BertPooler模块的全连接层只接收BertEncoder模块中第一个token（[CLS]）对应的输出，因为BertPooler模块用于解决序列级任务，而序列级任务只会用到[CLS]位置对应的输出
        first_token_tensor = hidden_states[:, 0]
        pooled_output = self.dense(first_token_tensor)
        pooled_output = self.activation(pooled_output)
        return pooled_output #[bs, hid_size]
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True):
        ...
        extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)   # [bs, 1, 1, seqlen] 增加维度
        ...
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0    # padding的token置为-10000，exp(-1w)=0
