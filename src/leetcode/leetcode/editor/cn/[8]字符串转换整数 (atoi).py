...
class Automoton:
    def __init__(self):
        ...
        self.signed = 1
        self.stats = 'start'
        self.table = [['start', 'signed', 'num', 'end'],
                      ['end', 'end', 'num', 'end'],
                      ['end', 'end', 'num', 'end'],
                      ['end', 'end', 'end', 'end']
                      ]
    ...
    def get(self, c):
        self.stats = self.table[self.stats_index()][self.get_col(c)]
        if self.stats == 'num':
            self.ans = self.ans * 10 + int(c)
            if self.signed == 1:
                self.ans = min(self.ans, 2 ** 31 - 1)
            else:
                self.ans = - max(-self.ans, -2 ** 31)
        elif self.stats == 'signed':
            if c == '-':
                self.signed = -1
...
class Solution:
    def myAtoi(self, s: str) -> int:
        ...
        for c in s:
            automoton.get(c)
        return automoton.signed * automoton.ans
