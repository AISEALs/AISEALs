...
if FLAGS.model_type == "fasttext":
    ...
    model = load_model("model/fasttext_model_{}.bin".format(FLAGS.model_version))
    label_classes = model.get_labels()
else:
    tfidf_model_path = os.path.join(dirname, "model/tfidf_model_{}.sav".format(FLAGS.model_version))
    tfidf_model = pickle.load(open(tfidf_model_path,  'rb'))

    label_encoder_model_path = os.path.join(dirname, "model/lr_label_model_{}.sav".format(FLAGS.      model_version))
    label_encoder_model = pickle.load(open(label_encoder_model_path, 'rb'))

    logistic_model_path = os.path.join(dirname, "model/lr_model_{}.sav".format(FLAGS.model_version))
...
yali_words = read_rule_words(os.path.join(dirname, "data/shenghuo_yali_words.txt"))
...
def life_predict_labels(contents):
...
if __name__ == '__main__':
    ...
    y_predict = life_predict_labels(contents)
    print(y_predict)
