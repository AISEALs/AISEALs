...
def file_based_input_fn_builder(tfrecord_pattern, seq_length, is_training,
    ...
    def input_fn(params):
        ...
        d = d.interleave(tf.data.TFRecordDataset, cycle_length=10, block_length=1)
...
def serving_input_receiver_fn():
    ...
    return tf.estimator.export.build_parsing_serving_input_receiver_fn(name_to_features, seq_length)
...
def main(_):
    ...
    processor = get_processor(FLAGS.base_dir, task_name, FLAGS.task_id, use_hdfs=False)
...
def test():
    train_pattern = os.path.join("/Users/jiananliu/AISEALs/data/text_classification/tribe_labels_20190521/tfrecord", "eval*.tfrecord")
    params = HParams(
        train_batch_size=FLAGS.train_batch_size,
        eval_batch_size=FLAGS.eval_batch_size)
