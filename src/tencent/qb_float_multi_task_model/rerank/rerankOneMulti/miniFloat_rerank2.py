...
for idx in range(0, NUM_IN_FLUSH):
    ...
    for start_slot in dense_slots:
        ...
        basic_embeddings = tf.get_variable("basic_embed_" + str(start_slot) + "_" + str(idx),
                                           shape=[tower_number, EMBEDDING_DIM],
                                           dtype=tf.float32,
                                           initializer=dense_normal_init) # M * D

        video_time_bins = tf.squeeze(math_ops._bucketize(dense_pl, boundaries=bins), axis=1)  # N * 1
        video_time_bins = tf.one_hot(video_time_bins, tower_number) # N * M
        basic_embed = tf.matmul(video_time_bins, basic_embeddings) # N * D

        single_dense_fea.append(basic_embed)
    singe_dense_scores = tf.concat(single_dense_fea, axis=1, name="dense_score_fea_%d" % idx) # N * (D * 4)
...
# 单个item
size_of_concat_emb = EMBEDDING_DIM * (num_item_feature + num_user_feature) + 1 + EMBEDDING_DIM * 4
