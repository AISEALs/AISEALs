...
import torch
from pytorchD.utils import cuda, precision
...
class DynamicVocabulary:
    ...
    def __init__(
            self,
            static_vocab: Vocabulary,
            max_size: int = 0
    ):
        #notice： 这里PriorityQueue引用heapq，不能多进程迭代dataset
        from tools.priority_queue import PriorityQueue
        self.queue = PriorityQueue(maxsize=max_size)
        # self.queue = None
        self.queue.init_from_dict(static_vocab.idx)

    def lookup_all(self, nested_values):
        res, counts, replace_values = self.lookup_all_internal(nested_values)
        return res, counts, replace_values

    def lookup_all_internal(self, nested_values):
        def lookup(value):
            return self.queue.look_up(value)
        ...
        if not should_iter(nested_values):
            ...
            old_values = []
            for value in nested_values:
                v, count, old_value = lookup_value(value)
                indices.append(v)
                counts.append(count)
                old_values.append(old_value)
            return indices, counts, old_values
...
class VocabBuilder:
    ...
    def __init__(self, maxsize, delimiter=" "):
        self._counter = Counter()
        # 动态词库 不需要unk，静态词库需要unk
        self.use_unk = True
    ...
    def add(self, value) -> None:
        ...
        if value not in self._counter and len(self._counter) > self.maxsize:
            return
