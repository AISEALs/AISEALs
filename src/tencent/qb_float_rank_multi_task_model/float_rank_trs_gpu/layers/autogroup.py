...
class AutoGroupLayer(BaseLayer):
    ...
    def call(self, inputs, training) -> tf.Tensor:
        ...
        self.selections, self.soft_probs = self._differentiable_sampling(training)
        # for name, t in self.get_features_in_groups().items():
        #     tf.print('perturb_on:', self.perturb_on, name, t)
    ...
    def _differentiable_sampling(self, training) -> tf.Tensor:
        ...
        Returns:  A 2-D ``tf.Tensor``, the selection masks of groups. shape: [slot_num, group_num]
        ...
        logits = tf.where(perturbed_flag, gumbel_logits, self.structure_logits)  # [slot_num, group_num, 2]
        ...
        selected_probs = tf.squeeze(tf.linalg.matmul(probs, self._choice_matrix))  # [slot_num, group_num]
        selected_probs_per_group = tf.transpose(selected_probs, perm=[1, 0])  # [group_num, slot_num]
        _, top_k_indices = tf.math.top_k(selected_probs_per_group, k=self.order_raw)  # [group_num, top_k]
        # top_k-indices format is [[0, 2], ...] => reduce max result is [[1, 0, 1, 0], ...]
        one_hots = tf.one_hot(top_k_indices, self.slot_num)  # [group_num, top_k, slot_num]
        hard_top_k_selections = tf.reduce_max(one_hots, axis=1)  # [group_num, slot_num]
        ...
        return trainable_selections, soft_top_k_selections
    ...
    def _make_interactions(self, embed_matrix) -> tf.Tensor:
        ...
        )  # [batch_num, emb_dim, slot_num] * [slot_num, group_num] => [batch_num, emb_dim, group_num]
    ...
    def get_selections_info_in_groups(self) -> (tf.Tensor, Dict[str, tf.Tensor]):
        features_in_groups = {}
        if self.selections is not None:
            hard_selections = tf.split(self.selections, num_or_size_splits=self.group_num, axis=1)
            soft_selections = tf.split(self.soft_probs, num_or_size_splits=self.group_num, axis=1)
            for i, (hard_selection, soft_selection) in enumerate(zip(hard_selections, soft_selections)):
                group_name = "order_{}_group_{}".format(self.order_raw, i)
                features_in_groups[group_name] = (tf.reshape(tf.where(tf.squeeze(hard_selection) > 0.5), [-1]),
                                                  soft_selection)
        return self.perturb_on, features_in_groups
